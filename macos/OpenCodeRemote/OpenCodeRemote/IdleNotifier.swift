...
final class IdleNotifier {
    ...
    private let logger = OSLog(subsystem: "ai.opencode.remote", category: "IdleNotifier")
    private let controlFileURL: URL
    private let session: URLSession
    private var pollTimer: DispatchSourceTimer?
    private let queue = DispatchQueue(label: "ai.opencode.remote.idlenotifier", qos: .utility)
    ...
    private func showLocalNotification(completion: CompletedSession) {
        ...
        content.title = "Tarea de OpenCode terminada"
        let project = completion.projectName ?? completion.directory ?? "tu proyecto"
        content.body = "\(project): OpenCode terminó mientras no estabas. Revisa Telegram o vuelve a la laptop."
        content.sound = .default
        let request = UNNotificationRequest(
            identifier: "opencode.completion.\(completion.sessionId)",
            content: content,
            trigger: nil
        )
